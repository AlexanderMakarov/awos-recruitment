...
**State the run's agent roster too** — one line, `Agents: 11 dispatched (haiku 5, sonnet 5, opus 1), max depth 1, none named`. Enumerating is the point: it catches a named background agent, an agent dispatched without a `model`, a depth-2 fan-out, or a fourth engine invented along the way. Each of those is expensive, and each is invisible unless counted. If the roster comes out wrong, say so at the gate rather than burying it — the review is still fine, but the next run shouldn't repeat it.

**Pre-gate protocol — the draft is the step 4 file, not your memory.** Present it by printing the file's full content as message text, then call `AskUserQuestion` — and always include the file path in the question itself ("full draft in `review/pr-<N>-draft.md`"), so even if the print gets squeezed out, the user opens the draft in their editor from the path alone. The failure this guards against is composing the draft in thinking and then gating on "the draft is above" while the message contains nothing — a memory of having printed is not evidence, and it is convincing from the inside. Only the `Write` call from step 4 and text visible in this turn count. If there is no step 4 file, you have no draft: go back and write it.
