...
**Already on the PR's head branch?** Then don't create anything — `git pull --ff-only` to the tip and work in place. A worktree only earns its cost when a checkout would otherwise disturb a *different* branch; when you're already on the right one it's just clutter.
...
```sh
git worktree add --detach <dir>   # <dir> defaults to the sibling ../<repo>-pr-<NUM>
cd <dir>
gh pr checkout <NUM>              # PR head branch, fork-aware
git pull --ff-only               # move to the tip
```

`<dir>` defaults to the sibling `../<repo>-pr-<NUM>`; if the user names a target directory, use that instead. Tell the user the worktree path. Remove it when done (`git worktree remove <path>`) unless they want to keep it.
