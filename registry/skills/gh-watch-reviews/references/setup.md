...
Build `config` via ONE `AskUserQuestion` call with these four questions:
...
**Migration** (config written by an older skill version, `review_target` missing): SKILL.md step 1 asks the one missing question and persists it; don't re-run the full interview.
